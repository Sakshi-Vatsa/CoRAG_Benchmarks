...
def format_documents_for_final_answer(
    ...
    selected_doc_ids: List[str] = context_doc_ids[:20]
    documents: List[str] = [format_input_context(corpus[int(doc_id)]) for doc_id in selected_doc_ids]

    max_per_ctx_length: int = int(3072 / max(20, 1) * 1.2)
    ...
    documents = _apply_context_placement_strategy('backward', documents)
